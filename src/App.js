...
class App extends Component {
  ...
  decrementSec = () => {
    ...
    this.setState(prevState => ({
      timer: timer > 0 ? prevState.timer - 1 : 0,
    }))
    if (timer === 1) {
  ...
  filterImage = () => {
    ...
    const filterMatchImage = imagesList.filter(
      each => each.category.toLowerCase() === activeTab.toLowerCase(),
    )
    return filterMatchImage
  }

  resetGame = () => {
    this.setState({isGameProgress: true, score: 0, timer: 60})
  }

  imageTab = id => {
    const {randomImage, timer} = this.state
    const randomIndex =
    ...
    if (id !== randomImage.id || timer === 1) {
      // Game is over when the timer reaches zero
      this.setState({isGameProgress: false})
    }
    this.setState({randomImage: randomIndex})
  }

  renderCard = () => {
    const {score} = this.state
    return <WinGame isScore={score} onPlayAgain={this.resetGame} />
  }

  renderMatch = () => {
    const {activeTab, randomImage} = this.state
    const filterMatchImage = this.filterImage()
        ...
        <ul className="tab-container">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={each.tabId === activeTab}
            />
          ))}
        </ul>
        <div className="match-image-container">
          <ul className="match-image-list">
            {filterMatchImage.map(each => (
              <MatchItem
                matchImage={each}
                key={each.id}
  ...
  render() {
    const {timer, score, isGameProgress} = this.state
    return (
      <>
        <nav className="navbar-container">
          ...
          <ul className="navbar-scoreTimer-container">
        ...
        <div className="match-main-container">
          <div className="match-container">
            {isGameProgress ? this.renderMatch() : this.renderCard()}
          </div>
        </div>
      </>
    )
